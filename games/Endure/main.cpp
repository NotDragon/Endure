	...
	while (!WindowShouldClose() && shouldBePlaying)
	{
		...
		for (auto door : doors)
		{
            ...
            BeginTextureMode(screen);
            {
                ...
                DrawText(std::to_string(atan2(character.y - GetMouseY(), character.x - GetMouseX())).c_str(), 10, 70,
                         22, DARKGREEN);
		...
		for (int x = 0; x < row.size(); x++)
		{
			...
			if (style == "\"normal\"")
			{
				...
				else if (tile == 'd')
				{
					...
					{
						doors.emplace_back(Rectangle{BLOCK_WIDTH * x - textures["DOOR_WOODEN"].width * 1.0f, BLOCK_HEIGHT * (y + 0.43f) - textures["DOOR_WOODEN"].height * 1.0f, BLOCK_WIDTH * 3.0f, BLOCK_HEIGHT * 2.0f}, levelDoors[doorCounter]["roomid"]);
					...
					{
						doors.emplace_back(Rectangle{BLOCK_WIDTH * x - textures["DOOR_FRONT"].width * 1.0f, BLOCK_HEIGHT * (y + 0.43f) - textures["DOOR_FRONT"].height * 1.0f, BLOCK_WIDTH * 3.0f, BLOCK_HEIGHT * 2.0f}, levelDoors[doorCounter]["roomid"]);
				...
				else if (tile == 'e' || tile == 'E')
				{
					...
					{
						enemies.emplace_back((x - 1) * BLOCK_WIDTH, (y + 0.43) * BLOCK_WIDTH, enemiesLevel[enemyCounter]["enemyid"]);
						enemyCounter++;
					}
				}
			}
			else if (style == "\"live_cave\"")
			{
				...
				else if (tile == 'd')
				{
					if (levelDoors[doorCounter]["isWooden"] == 1)
					{
						doors.emplace_back(Rectangle{BLOCK_WIDTH * x - textures["DOOR_WOODEN"].width * 1.0f, BLOCK_HEIGHT * y - textures["DOOR_WOODEN"].height * 1.0f, BLOCK_WIDTH * 3.0f, BLOCK_HEIGHT * 2.0f}, levelDoors[doorCounter]["roomid"]);
					...
					{
						doors.emplace_back(Rectangle{BLOCK_WIDTH * x - textures["DOOR_FRONT"].width * 1.0f, BLOCK_HEIGHT * y - textures["DOOR_FRONT"].height * 1.0f, BLOCK_WIDTH * 3.0f, BLOCK_HEIGHT * 2.0f}, levelDoors[doorCounter]["roomid"]);
				...
				{
					doors.emplace_back(Rectangle{BLOCK_WIDTH * x - textures["DOOR"].width * 1.0f, BLOCK_HEIGHT * y - textures["DOOR"].height * 1.0f, BLOCK_WIDTH * 3.0f, BLOCK_HEIGHT * 2.0f}, levelDoors[doorCounter]["roomid"]);
				...
				else if (tile == 'e' || tile == 'E')
				{
					...
					if (enemies.size() == enemyCounter)
					{
						enemies.emplace_back((x - 1) * BLOCK_WIDTH, y * BLOCK_WIDTH, enemiesLevel[enemyCounter]["enemyid"]);
						enemyCounter++;
					}
				}
			}
...
void DrawTextureWithCollider(float x, float y, Texture2D texture)
{
    ...
    DrawRectangleRec(Rectangle{BLOCK_WIDTH * x, BLOCK_HEIGHT * y, BLOCK_WIDTH * 0.3f, BLOCK_HEIGHT * 2.0f}, YELLOW);
    DrawRectangleRec(Rectangle{BLOCK_WIDTH * x, BLOCK_HEIGHT * y, BLOCK_WIDTH * 2.0f, BLOCK_HEIGHT * 0.3f}, GREEN);
#endif
}
